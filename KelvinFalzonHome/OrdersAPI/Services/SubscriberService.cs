...
namespace OrdersAPI.Services
{
    public class SubscriberService : IHostedService, IDisposable
    {
        ...
        {
            subscriptionName = new SubscriptionName(settings.Value.Project, "movies-sub");
            NotificationSubscriptionName = new SubscriptionName(settings.Value.Project, "upcomingMovies-Orders-sub");
        ...
        private async Task<SubscriberClient.Reply> HandleOrderConfirmation(PubsubMessage msg, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var client = new RestClient("http://localhost:5040/api/watchlists/");
                var request = new RestRequest($"/removeWatchlist?userId={order.UserId}&movieId={order.MovieId}");
        ...
        private async Task<SubscriberClient.Reply> HandleUpcomingNotification(PubsubMessage msg, CancellationToken cancellationToken)
        {
            ...
            var request = new RestRequest($"/byId?MovieId={movieId}");
            var response = await client.GetAsync(request);
            ...
            foreach (var item in watchArray)
            {
                ...
                //System.Diagnostics.Debug.WriteLine("This is in the DoWork: " + parsedGenre);
                var newrequest = new RestRequest($"/byMovieId?id={movieId}");
