...
namespace WatchListsAPI.Services
{
    public class SubscriberService : IHostedService, IDisposable
    {
        ...
        public SubscriberService(IOptions<GCPSettings> settings, WatchlistsService watchlistsService, PublisherService publisherService)
        {
            subscriptionName = new SubscriptionName(settings.Value.Project, "upcomingMovies-Watchlist-sub");
        ...
        {
            _timer = new Timer(async _ => await DoWork(_), null, TimeSpan.Zero,TimeSpan.FromSeconds(15));
